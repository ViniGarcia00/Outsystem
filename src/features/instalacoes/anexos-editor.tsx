...
import type { AnexoDTO } from "@/services/instalacao-registro.service";

import { excluirAnexoAction } from "./anexo-actions";
import { ACCEPT_ANEXO, MAX_POR_REGISTRO, formatarTamanho } from "./anexos";

/**
 * Anexos de um registro da cronologia (Sprint 4.3, ADR-0414).
 *
 ...
 * novo e existente terem exatamente a mesma regra.
 *
...
export function AnexosEditor({
  ...
  const [aExcluir, setAExcluir] = useState<AnexoDTO | null>(null);

  const base = `/instalacoes/${instalacaoId}/registros/${registroId}/anexos`;
  ...
  async function enviar(file: File) {
    ...
    try {
      const resp = await fetch(base, { method: "POST", body: corpo });
      if (resp.ok) {
  ...
  async function confirmarExclusao() {
    if (!aExcluir) return;
    const result = await excluirAnexoAction(instalacaoId, registroId, aExcluir.id);
    if (result.success) {
            ...
            {anexos.map((anexo) => {
              ...
              return (
                ...
                >
                  <a
                    href={`${base}/${anexo.id}`}
                    // O nome do arquivo já vai no Content-Disposition da rota;
                    // `download` aqui só reforça a intenção para o navegador.
