...
import type { CustoPosVendaValues } from "./registro-schema";

/**
 * Linhas de custo dentro do diálogo do registro (Sprint 4.6).
 *
 * Estado CONTROLADO pelo diálogo — o projeto não usa `useFieldArray` em lugar
 * nenhum, e manter a consistência evita um segundo padrão de array em
 * formulário.
 *
 * Não valida nada: a validação é do `registroPosVendaSchema`. Este componente
 * só edita. O total exibido vem de `totalDoRegistro` — nenhuma soma é feita
 * aqui.
 *
 * `categorias` vem por prop porque a Troca oferece custos de ENVIO e a OS, de
 * REPARO (ADR-0418). O componente não sabe qual dos dois está editando, e não
 * precisa saber.
 */
export function CustosEditorPosVenda({
  custos,
  categorias,
  onChange,
  disabled = false,
}: {
  custos: CustoPosVendaValues[];
  categorias: CategoriaCustoPosVenda[];
  onChange: (custos: CustoPosVendaValues[]) => void;
  disabled?: boolean;
}) {
  const alterar = (indice: number, patch: Partial<CustoPosVendaValues>) =>
  ...
  const adicionar = () =>
    onChange([
      ...custos,
      { categoria: categorias[0], descricao: "", valor: 0 },
    ]);
            ...
            >
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    alterar(indice, {
                      categoria: v as CategoriaCustoPosVenda,
                    })
                  }
                  disabled={disabled}
                >
